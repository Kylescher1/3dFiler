  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', zIndex: 20, pointerEvents: 'none' }}>
      <div style={{ pointerEvents: 'auto', display: 'flex', alignItems: 'center', gap: '12px', ...panelStyle, padding: '8px 14px' }}>
        <Link to={leftLink} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--primary)', fontWeight: 700, fontSize: '0.9rem', textDecoration: 'none', fontFamily: 'var(--font-display)', letterSpacing: '1px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px', flexWrap: 'wrap', fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--text-muted)' }}>
              {breadcrumbs.map((crumb, idx) => (
                <span key={crumb.id} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  {idx > 0 && <span style={{ color: 'rgba(0,0,0,0.15)' }}>/</span>}
                  <span style={{ color: idx === breadcrumbs.length - 1 ? 'var(--text-primary)' : 'var(--primary)' }}>
                    {crumb.title}
                  </span>
                </span>
              ))}
      ...
      <div style={{ pointerEvents: 'auto', display: 'flex', alignItems: 'center', gap: '8px', ...panelStyle, padding: '4px' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', background: 'rgba(0,0,0,0.04)', borderRadius: '8px', padding: '3px' }}>
          <button onClick={onOpenViewer} style={{ width: '88px', height: '30px', borderRadius: '6px', border: 'none', background: mode === 'viewer' ? 'rgba(185, 28, 28, 0.15)' : 'transparent', color: mode === 'viewer' ? 'var(--primary)' : 'var(--text-muted)', cursor: mode === 'viewer' ? 'default' : 'pointer', fontSize: '0.78rem', fontWeight: mode === 'viewer' ? 600 : 500, fontFamily: 'var(--font-mono)', letterSpacing: '0.5px' }}>
            VIEWER
          </button>
          <button onClick={onOpenWiki} style={{ width: '88px', height: '30px', borderRadius: '6px', border: 'none', background: mode === 'wiki' ? 'rgba(185, 28, 28, 0.15)' : 'transparent', color: mode === 'wiki' ? 'var(--primary)' : 'var(--text-muted)', cursor: mode === 'wiki' ? 'default' : 'pointer', fontSize: '0.78rem', fontWeight: mode === 'wiki' ? 600 : 500, fontFamily: 'var(--font-mono)', letterSpacing: '0.5px' }}>
            WIKI
          </button>
        </div>
      </div>
