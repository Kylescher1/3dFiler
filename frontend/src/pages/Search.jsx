...
function EmptySearchState() {
  return (
    <div className="card" style={{ padding: '1.5rem', background: 'linear-gradient(135deg, #111, #0b0f13)' }}>
      <h2 style={{ color: '#e0e0e0', marginBottom: '0.7rem' }}>Search your 3D wiki</h2>
      <p style={{ color: '#888', lineHeight: 1.6, maxWidth: 740 }}>Find models, wiki page text, tags, descriptions, and individual points of interest from one place. Try searching for a part name, material, project, or tag.</p>
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', flexWrap: 'wrap' }}>
        {['anatomy', 'reference', 'mechanism', 'draft', 'nested'].map(term => <span key={term} style={{ color: '#4fc3f7', background: '#0f1f2a', border: '1px solid #1e3a4c', borderRadius: 999, padding: '0.25rem 0.65rem', fontSize: '0.78rem' }}>{term}</span>)}
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div>
          <p style={{ color: '#4fc3f7', textTransform: 'uppercase', letterSpacing: '0.14em', fontSize: '0.72rem', fontWeight: 800, marginBottom: '0.35rem' }}>Knowledge Search</p>
          <h1 className="page-title" style={{ marginBottom: '0.3rem' }}>Find Anything</h1>
          <p style={{ color: '#888' }}>Search models, wiki content, POIs, tags, filenames, and notes.</p>
        </div>
        <Link to="/dashboard" className="btn btn-secondary">Back to Library</Link>
      </div>
      ...
      {!loading && !searched && <EmptySearchState />}

      {!loading && searched && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <p style={{ color: '#888' }}>{total} result{total === 1 ? '' : 's'} for <span style={{ color: '#e0e0e0' }}>“{searchParams.get('q')}”</span></p>
                ...
                {results.models.map(m => {
                  ...
                  return (
                    <Link key={m.id} to={`/model/${m.id}`} className="card" style={{ textDecoration: 'none', color: 'inherit', position: 'relative', overflow: 'hidden' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.75rem', color: '#666', fontSize: '0.72rem', marginTop: '0.7rem' }}>
                        <span>{m.summary?.poiCount || 0} POIs</span><span>{m.summary?.nestedCount || 0} links</span><span>{m.summary?.completionScore || 0}% complete</span>
                      </div>
                    </Link>
                  )
                })}
